...
class TextProcessor:
    ...
    def __init__(self):
        ...
        self.AVAILABLE_COMMANDS = {
            ...
            "commands": {
                ...
                "full-off": ["отключись полностью", "отключить полностью", "полное отключение"],
                "off": ["отключись", "отключение"],
                "on": ["привет", "включись", "включение"],
                "time": ["сколько времени", "текущее время"],
                "date": ["какой сегодня день", "сегодняшняя дата"],
                "weather-now": ["какая сейчас погода", "текущая погода"],
                "course": ["курс валют"],
                "features": ["что ты умеешь", "возможности"],
                "create-scenario": ["создай сценарий", "добавь сценарий", "добавление сценария"],
                "execute-scenario": ["запусти сценарий", "исполни сценарий", "исполнение сценария"],
                "delete-scenario": ["удали сценарий", "удаление сценария"]
            }
        }
    ...
    def match_command(self, command: str, ignore_all: bool):
        ...
        :return: Если в тексте не найдено обращения к голосовому помощнику, будет возвращено [[ None; None ]].
                 ...
                 команды, будет возвращён служебный ключ [[ "С-N-F", None ]].
                 Если хотя бы в одной из команд будет допущена ошибка формата (например, рекурсивное создание сценария в сценарие), то ВСЕ команды будут проигнорированы и возвращён ключ [[ "F-E", None ]], извещающий об ошибке формата. В случае успешного распознавания команд
        ...
        for it in range(command_size):
            ...
            if len(selected_actions) and selected_actions[0][0] == "create-scenario" \
                    and picking_result[0][-8:] == "scenario":
                return [
                    ["F-E", ["create-scenario", "При создании сценария запрещена любая работа с другими сценариями."]]
                ]

            if picking_result[0] == "create-scenario" and len(selected_actions):
                return [
                    ["F-E", ["create-scenario", "Произнесение других команд перед созданием сценария недопустимо."]]
                ]
