...
def get_currency_course(**kwargs):
    ...
    try:
        ...
        return Response(
            text=get_phrase("COURSE_REQUEST_ERROR"),
            is_correct=False
        )
...
def get_weather_now(**kwargs):
    ...
    city = global_context.CITY if kwargs["info"] is None else kwargs["info"]
    ...
    try:
        ...
        if data["cod"] == "404":
            return Response(
                header=get_phrase("WEATHER_NOT_FOUND_ERROR"),
                text=city,
                is_correct=False,
        ...
        def prepare_result():
            ...
            temp1 = cur_weather if global_context.weather_celsium else int(cur_weather * 9 / 5) + 32
            temp1_pf = " " + declension(temp1, 'фаренгейт')
            result += (f"Температура: {'+' if temp1 > 0 else ''}{ temp1 }"
                       f"{'°' if global_context.weather_celsium else temp1_pf} \n")

            temp2 = feel if global_context.weather_celsium else int(feel * 9 / 5) + 32
            temp2_pf = " " + declension(temp2, 'фаренгейт')
            result += (f"Ощущается как {'+' if temp2 > 0 else ''}{ temp2 }"
                       f"{'°' if global_context.weather_celsium else temp2_pf} \n")

            result += f"Влажность: { humidity }% \n"

            prs = int(pressure * 3 / 4) if global_context.weather_mmHg else pressure
            prs_phrase = f"милли{ declension(prs, 'метр') } ртутного столба" if global_context.weather_mmHg \
        ...
        return Response(
            text=get_phrase("WEATHER_REQUEST_ERROR"),
            is_correct=False
        )
